...
- (void)detectImage:(UIImage *)image {
    ...
    std::vector<LaneDetect::Lanes> result;
    if (self.USE_MODEL == W_NANODET) {
        result = self.lanedetct->detect(image, self.threshold, self.nms_threshold);
    }
    __weak typeof(self) weakSelf = self;
    dispatch_sync(dispatch_get_main_queue(), ^{
        if (weakSelf.USE_MODEL == W_NANODET) {
            weakSelf.imageView.image = [weakSelf drawBox:weakSelf.imageView image:image
                                                   Lanes:result];
        }
...
#pragma mark 创建模型
- (void)createModel {
    if (!self.lanedetct && self.USE_MODEL == W_NANODET) {
        NSLog(@"new NanoDet");
        self.lanedetct = new LaneDetect("mlsd_with_max_sigmoid.mnn",self.USE_GPU);
    }
}

- (void)releaseModel {
    NSLog(@"release model");
    delete self.lanedetct;
}
...
- (UIImage *)drawBox:(UIImageView *)imageView image:(UIImage *)image
